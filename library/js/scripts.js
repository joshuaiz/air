    ...
    var q = /\s*([\-\w ]+)[\s\/\:]([\d_]+\b(?:[\-\._\/]\w+)*)/,
        ...
        k = {
            parse: function(b, d) {
                ...
                if (!b) return a
                for (
                    var c, e, g = b.split(/[()]/), f = 0, k = g.length;
                    f < k;
                    f++
                )
                    if (f % 2) {
                ...
                w.exec(b)
                    ? ((a.mobile = RegExp.$1),
                      s.exec(b) &&
                          (delete a[a.mobile],
                          (a.blackberry =
                              a.version || RegExp.$3 || RegExp.$2 || RegExp.$1),
                          RegExp.$1
                              ? (a.mobile = 'blackberry')
                              : '0.0.1' === a.version &&
                                (a.blackberry = '7.1.0.0')))
                    : x.exec(b)
                    ? ((a.game = RegExp.$1),
                ...
                if (a.applewebkit)
                    (a.webkit = a.applewebkit),
                        delete a.applewebkit,
                        a.opr &&
                            ((a.opera = a.opr), delete a.opr, delete a.chrome),
                        a.safari &&
                            (a.chrome ||
                            a.crios ||
                            a.fxios ||
